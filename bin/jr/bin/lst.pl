...
# 2004-05-12 downloaded from http://search.cpan.org/~sdague/ppt-0.12/
#            renamed lst to match existing name
#            (and avoid any possible confusion with ls, however unlikely).
#            suppressed printing "total xxxx" when, e.g., ls -l filename
#            as per UNIX behavior.
#            (added extra param to List; didn't see better way.)
...
#
# $Revision: 1.1.1.1 $ $Date: 2001/06/06 08:54:45 $
...
sub DirEntries {
	...
	# 2008-03-26 RAO
	# perl, v5.10.0 now gives warning on original closedir
	#   closedir(DH) if (defined(DH);
	# closedir() attempted on invalid dirhandle DH at ...
	# which probably wasn't correct anyway, since DH is defined (*main::DH)
	# so split it up a bit and change closedir.
	my $opened = opendir(DH, $_[0]);
	if (!$opened || exists($Options{'d'})) {
		if (-e $_[0]) {
			closedir(DH) if ($opened);
			push(@Entries, $_[0]);
...
sub List {
	...
	my $IsFile = shift;	# whether is simple file (vs. directory)
...
#	if (exists($Options->{'l'}) || exists($Options->{'s'})) {
	if (!$IsFile && 
	    (exists($Options->{'l'}) || exists($Options->{'s'}))) {
...
if ($#ARGV < 0) {
	List('.', 0, \%Options, 0, DirEntries(\%Options, "."));
